...
const HeaderBar: React.FC<Props> = ({
  ...
  const applicantPill = (() => {
    ...
    if (active)
      return `${pillBase} text-white bg-red-600 border-red-600 ring-2 ring-red-300`;
    if (clickable)
      return `${pillBase} text-red-700 bg-red-50 border-red-200 hover:bg-red-100`;
    // visible uniquement si isApplicant === true
    return `${pillBase} text-red-700 bg-red-50 border-red-200`;
  })();
  ...
  const tenantPill = (() => {
    ...
    if (active)
      return `${pillBase} text-white bg-green-500 border-green-500 ring-2 ring-green-300`;
    if (clickable)
      return `${pillBase} text-green-700 bg-green-50 border-green-200 hover:bg-green-100`;
    // visible uniquement si isTenant === true
    return `${pillBase} text-green-700 bg-green-50 border-green-200`;
  })();
  ...
  return (
    <div className="flex justify-between items-center">
      {/* Pills Demandeur / Locataire — afficher uniquement les actifs */}
