...
export default function QuickTagGrid({
  ...
  person: Person;
  ensureBlock: <K extends keyof Person>(
    pid: string,
    key: K,
    factory: () => any
  ) => void;
  patchBlock: <K extends keyof Person>(
    pid: string,
    key: K,
    patch: Partial<any>
  ) => void;
        ...
        {TAGS.map(({ key, label }) => {
          ...
          return (
            <button
              ...
              onClick={() =>
                active
                  ? clearBlock(person.id, key)
                  : ensureBlock(person.id, key, () => ({
                      monthly: 0,
                      months: 12,
                      includeInRDU: true,
                    }))
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
        {TAGS.map(({ key, label }) => {
          ...
          return (
            <div
              key={String(key)}
              className="rounded-md border px-3 py-2 bg-slate-50"
            >
              <div className="flex items-center justify-between mb-2">
                ...
                <Button
                  variant="ghost"
                  size="sm"
                  onClick={() => clearBlock(person.id, key)}
                >
                  Supprimer
                </Button>
              </div>
              <div className="grid grid-cols-1 md:grid-cols-4 gap-2 items-end">
                ...
                <Field label="Remarque">
                  <Input
                    value={block.remark ?? ""}
                    onChange={(e) =>
                      patchBlock(person.id, key, { remark: e.target.value })
                    }
